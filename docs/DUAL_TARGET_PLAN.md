...
- Separate live AutoGPTQ TP=2, deterministic W4A16/MARLIN, and FP8/TRITON server-to-ring-to-optimizer transactions pass with teacher/training state separation, transactional ring acknowledgement, and exact tied vocabulary preservation. The FP8 branch additionally passed one retained eight-example/eight-step micro-pilot with exact response/request/release ledgers and draft/AdamW round-trip. Continuous scheduling, larger matched pilots, serving integration, quality, acceptance, latency, memory, and throughput remain unverified; the micro-pilot does not select geometry.
